# Wedding Video Skill 实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将现有 `skill.md` 重写为正式的 Claude Code Skill，增加自动扫描和交互式问答能力。

**Architecture:** 仅修改 `skill.md` 一个文件。在现有内容前增加 YAML 元数据头部和触发条件，将原 Phase 1（项目探索）替换为自动扫描 + 交互式问答，后续 Phase 保持不变。

**Tech Stack:** Markdown + YAML frontmatter (Claude Code Skill 格式)

---

### Task 1: 重写 skill.md — 元数据头部 + 角色定位
...
完整的 skill.md 内容如下（一次性写入整个文件）：内容见下方完整文件。

用 Write 工具写入 `/Users/wanglian/Projects/video/wedding/skill.md`，内容：
...
description: >
  ...
  使用 Remotion 一键生成婚礼回忆视频。支持 7 种画面布局、
  12 种动画、多种装饰特效和配色方案。
trigger:
  keywords:
    - 生成视频
  ...
  autoDetect:
    ...
    check: "扫描 ./resource/image/ 下子目录数量，统计含图片的目录；检查 ./resource/music/ 是否存在"
allowed-tools:
  - Bash
  - Read
  - Write
...
2. **自动检测触发**：当前工作目录下，`./resource/image/` 中存在 ≥2 个包含图片文件的子目录，且 `./resource/music/` 目录存在
...
1. `ls ./resource/image/` → 列出所有子目录
2. 对每个子目录，`ls ./resource/image/<目录名>/` 统计文件数量
3. `ls ./resource/music/` → 列出所有音乐文件
4. **将扫描结果汇总展示给用户**，格式：
  ...
  "categories": [
    {
      "name": "directory_name",
      "title": "章节标题",
      "folderPath": "./resource/image/目录名",
      "emphasis": false
    }
  ],
  "music": "./resource/music/selected-song.mp3",
  "duration": <用户确认的秒数>,
  "style": "<配色风格>",
  "ending": {
    "names": "新郎 & 新娘",
...
1. 读取 resource/image 下选中的每个子目录
2. 列出每个子目录中的图片文件
3. 获取每张图片的宽高（使用 `sips -g pixelWidth -g pixelHeight` 或从文件元数据读取）
4. 按 `photoManifest.json` 格式组织数据：
   - `name`: 目录名
   - `title`: 用户确认的章节标题
   - `photos`: 该目录下所有图片文件，每个含 `path`（相对于 resource/image/目录/）、`width`、`height`
   - `emphasis`: 用户确认的是否强调
...
2. 使用 `npx remotion render WeddingVideo out/wedding.mp4` 渲染
...
```bash
wc -l /Users/wanglian/Projects/video/wedding/skill.md
```
预期：文件行数 >= 200
...
```bash
git add skill.md docs/superpowers/specs/2026-05-12-wedding-video-skill-design.md docs/superpowers/plans/2026-05-12-wedding-video-skill.md
git commit -m "feat: redesign skill.md as installable Claude Code Skill with auto-scan and interactive Q&A"
```
